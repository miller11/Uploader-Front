<template>
  <div class="col-lg-4 col-sm-6 portfolio-item">
    <div class="card h-100">
      <a @click="showModal"><img :src="album.coverPhoto.url" :alt="album.coverPhoto.name" class="card-img-top cover-photo"></a>
      <div class="card-body">
        <h4 class="card-title">
      ...
      <div class="card-footer" v-if="isPhotoOwner && isEditScreen">
        <div class="float-right p-1">
          <a class="text-danger" href="" @click.prevent="deleteAlbum"><font-awesome-icon icon="trash"></font-awesome-icon></a>
        </div>

        <div class="float-right p-1">
          <router-link :to="{name: 'albumLink', params: { albumKey: albumKey }}"><font-awesome-icon icon="wrench"></font-awesome-icon></router-link>
        </div>
      </div>
    </div>
      ...
      deleteAlbum() {
        ...
        if(confirm("Are you sure you wish to delete this album?")) {
          ...
          for(let key in self.photos) {
            ...
            stAlbumPhotosRef(self.albumKey).child(self.photos[key].name).delete().then(function () {
              // Files deleted successfully
              console.log(self.photos[key].name + " has been deleted");
